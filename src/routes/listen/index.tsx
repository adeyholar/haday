...
function ListenPage() {
  ...
  const iRef = useRef(i);
  const item = list[i];
  const clip = item?.id ? vocabClip(item.id) : undefined;
  const lemmaClip = clip?.kind === "lemma" ? clip : undefined;
  ...
  return (
    <>
      ...
      <div className="rounded-[var(--radius-xl)] bg-card px-5 py-10 text-center shadow-[var(--shadow-border)]">
        ...
        {lemmaClip ? <p className="mt-3 text-sm text-primary">From the reading · {vocabClipLabel(lemmaClip)}</p> : null}
