...
export function GrammarPlay({ track, unitId }: { track: GrammarTrack; unitId: number }) {
  ...
  function startQuiz() {
    if (!unit) return;
    const built = buildGrammarQuiz(track, unitId).map((item, n) => ({ ...item, key: `${track.id}-${unitId}-${n}` }));
    setItems(built);
    setI(0);
    setPicked(null);
    setHeld(new Set());
  ...
  function next() {
    ...
    if (i + 1 >= nextItems.length) {
      ...
      complete(track.id, unitId, {
        stars: starsFromGrammarScore(scorePct),
        score: scorePct,
      ...
      <>
        ...
        <Panel className="mt-3">
          ...
          <ul className="mt-3 space-y-3">
            {unit.samples.map((s, n) => (
              <li key={`${s.word}-${n}`} className="rounded-[var(--radius-md)] bg-surface px-3 py-3">
        ...
        <Panel className="mt-3">
          ...
          <ul className="mt-3 space-y-4">
            {learnUnitVerses(unit.verses, unit.samples).map((v, n) => (
              <TanakhLearnVerse key={`${v.ref}-${v.hit}-${n}`} verse={v} kind={track.kind} samples={unit.samples} />
            ))}
          </ul>
        </Panel>
    ...
    return (
      <Panel className="text-center">
        ...
        <div className="mt-4 flex flex-col gap-2">
          {passed && unitId < unitMax ? (
            <Link
              to="/game/lessons/$track/$unit"
